...
object SingleRequest {
  ...
  def main(): Unit = {
    ...
    val r2 = http.batch(Seq[BatchRequest](
      "https://test.k6.io",
    ...
    val r4 = http.batch(Map[String, BatchRequest](
    ...
    all.foreach(r => check(r, Checkers("ok" -> Check[Response](res => res.status >= 200 && res.status < 400))))
